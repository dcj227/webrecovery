...
void SignalHandle(const char* data, int size)
{
	...
	// 所以如上面的记录到文件，也需要>以追加模式方可），所以这里发邮件或短信不是很适合，不过倒是可
	// 以调用一个 SHELL 或 PYTHON 脚本，而此脚本会先 sleep 3秒左右，然后将错误信息通过邮件或短信
	// 发送出去，这样就不需要监控脚本定时高频率执行，浪费效率了。
}
...
GLogHelper::GLogHelper(char* program)
{
    ...
    //google::SetLogDestination(google::GERROR,"log/error_");    //设置 google::ERROR 级别的日志存储路径和文件名前缀
    ...
    google::SetLogDestination(google::GERROR,LOGDIR"/ERROR_");		//设置 google::ERROR 级别的日志存储路径和文件名前缀
